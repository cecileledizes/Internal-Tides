...
umax = maximum(abs, u′_t[end][div(Nx, 2), :, :])
vmax = maximum(abs, v′_t[end][div(Nx, 2), :, :])
wmax = maximum(abs, w_t[end][div(Nx, 2), :, :])
...
u′ₙ = @lift interior(u′_t[$n], div(Nx, 2), :, :)
v′ₙ = @lift interior(v′_t[$n], div(Nx, 2), :, :)
wₙ = @lift interior(w_t[$n], div(Nx, 2), :, :)
