...
include("functions/grid_spacings.jl") # haven't tried using these with the nondimensional simulation
...
@inline function create_nondim_simulation(stop_time::Number, output_folder, simulation_parameters::NamedTuple)
    ...
    h₀ = (sp.δ * sp.H)
    # width = (h₀ * exp(-1/2) / sp.E) * sqrt(((sp.B^2) - 1) / (1 - (sp.β^2))) # width calculated from the slope of the Gaussian
    # @info width # useful for double checking calculated width
    width = sp.width # can also just specify width along with other simulation parameters
    ...
    fp = (; sp..., f = coriolis.f)
    u_forcing = create_tidal_forcing(fp)
