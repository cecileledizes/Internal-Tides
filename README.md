...
The "functions" subfolder in "src" contains functions which produce relevant or potentially relevant forcings, closures, grid spacings, ocean bottom topographies, and simulation parameters. These functions are called and used in the `NAME_sim.jl` files. Changing the values of the parameters in `parameters.jl` and `nondim_parameters.jl` can be done in the jobscript. 
